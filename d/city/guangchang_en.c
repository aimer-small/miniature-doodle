// guangchang_en.c - Bilingual version of Central Plaza
// Demonstrates the bilingual room system
// Inherits BILINGUAL_ROOM instead of ROOM for Chinese/English support
...
LONG
	...
	set("long_en", @LONG
This is the center of the city, a wide plaza paved with bluestone.
Some idlers wander about, and performers often put on shows here.
You can see large shops to the east and south, with travelers from
all over the country coming and going. To the west is the Yangzhou
garrison barracks, and to the north is the governor's office.
In the center of the plaza stands an old peach tree (tree), thick
and sturdy with dense foliage (leaf). Several unknown little birds
dart about among the branches. Beside the big tree stands a newly
erected sign (sign).
LONG
	);
	...
	// English item descriptions can be added via item_desc_en
	set("item_desc_en/tree", "A large tree with dense foliage. You can't see what's above. The branches are laden with ripe peaches.\n");
	set("item_desc_en/leaf", "The leaves hang loosely from the tree, tempting you to pick one.\n");
	set("item_desc_en/sign", "Please protect the trees and plants. Vandalism will be severely punished! -- Yangzhou Government Notice\n");
...
// Helper function to get bilingual text for the viewer
string get_bilingual_text(object me, string cn, string en)
{
	if (!me) return cn;
	if (me->query("env/language") == "en" && en)
		return en;
	return cn;
}
			...
			switch (MONEY_D->player_pay(me, topay * 5)) {
				case 0:
					...
					if (money_ob = present("silver_money", me)) {
						if (topay) money_str = money_str + "、" + CHINESE_D->chinese_number(money_ob->query_amount()) + "两白银";
						else money_str = CHINESE_D->chinese_number(money_ob->query_amount()) + "两白银";
						money_ob->set_amount(0);
						topay = 1;
					}
					if (money_ob = present("coin_money", me)) {
						if (topay) money_str = money_str + "、" + CHINESE_D->chinese_number(money_ob->query_amount()) + "文铜钱";
						else money_str = CHINESE_D->chinese_number(money_ob->query_amount()) + "文铜钱";
						money_ob->set_amount(0);
						topay = 1;
					}
					if (topay)
...
int do_shake(string arg)
{
	...
	if (level > 100) {
		...
		if (!random(8) && query("peach_count")) {
			add("peach_count", -1);
			switch (random(4)) {
				case 0: peachroom = find_object(__DIR__"guangchangbei"); break;
				case 1: peachroom = find_object(__DIR__"guangchangxi"); break;
				case 2: peachroom = find_object(__DIR__"guangchangnan"); break;
				case 3: peachroom = find_object(__DIR__"guangchangdong"); break;
			}
