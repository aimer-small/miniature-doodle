...
void flee(object me,int idx,int team)
{
	...
	if( g>0 ) {
		...
		if(objectp(me2)) me2->clear_condition("job_busy");
                if(objectp(me1)) {me1->add("job_time/大清国海战",1);
                                me1->set("job_name","神龙岛海战");}
                if(objectp(me2)){ me2->set("job_name","神龙岛海战");
                             me2->add("job_time/大清国海战",1);}
...
db_exp = exp /3 + random(exp/2);
	...
	if(objectp(me1))
 {
   ...
   ext = me1->query("relife/exp_ext");
		...
		tell_object(me1, HIC"你仔细回想着刚才海战的过程，又额外获得了"+CHINESE_D->chinese_number(exp*ext/20)+"点经验！\n"NOR);
   ...
   if( me1->query_condition("db_exp") ) {
          me1->add("combat_exp", db_exp );
          	...
          	tell_object(me1,HBGRN"双倍经验奖励期间，你额外地增加了"+chinese_number(db_exp)+"点实战经验！\n"NOR);}
}
  ...
  ext = me2->query("relife/exp_ext");
		...
		tell_object(me2, HIC"你仔细回想着刚才海战的过程，又额外获得了"+CHINESE_D->chinese_number(exp*ext/20)+"点经验！\n"NOR);
   ...
   if( me2->query_condition("db_exp") ) {
          me2->add("combat_exp", db_exp );
          	...
          	tell_object(me2,HBGRN"双倍经验奖励期间，你额外地增加了"+chinese_number(db_exp)+"点实战经验！\n"NOR);}
