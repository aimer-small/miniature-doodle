...
protected mixed _query( mapping map, string *parts )
{
	mixed value;
	int i, s;
...
protected int _delete( mapping map, string *parts )
{
	if( sizeof( parts ) == 1 ) {
...
protected mixed _set( mapping map, string *parts, mixed value )
{
	if( sizeof( parts ) == 1 )
